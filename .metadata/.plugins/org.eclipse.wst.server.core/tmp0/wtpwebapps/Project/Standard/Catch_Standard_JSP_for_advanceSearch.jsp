...
<c:set var="t1_count" value="0"></c:set>
<c:set var="t2_count" value="0"></c:set>
<c:set var="price_begin" value="and ((price "></c:set>
<c:set var="price_end" value="))"></c:set>



<c:set var="t1_end" value=""></c:set>
<c:set var="t1_end1" value=""></c:set>
<c:set var="t2_end" value=""></c:set>
<c:set var="t2_end1" value=""></c:set>

<c:set var="sqlString" value="select * from Product P"></c:set>

<c:set var="type2" value=",(select e_name,count(*) c from Cata2 where Cata2 like N'%"></c:set>
<c:set var="type3" value=",(select e_name,count(*) c from Cata1 where Cata1 like N'%"></c:set>


<c:forEach var="type" items='<%=request.getParameterValues("type[]")%>'>
	<fmt:parseNumber var="t1_count" type="number" value="${t1_count+1}" />
	<c:set var="sqlString" value="${sqlString}${type2}${type}%'"></c:set>
	<c:set var="type2" value=" or Cata2 like N'%"></c:set>
</c:forEach>
	<c:set var="t1_end" value=" group by E_name) C1"></c:set>
	<c:set var="sqlString" value="${sqlString}${t1_end}"></c:set>
	
<c:forEach var="type1" items='<%=request.getParameterValues("type1[]")%>'>
	<fmt:parseNumber var="t2_count" type="number" value="${t2_count+1}" />
	<c:set var="sqlString" value="${sqlString}${type3}${type1}%'"></c:set>
	<c:set var="type3" value=" or Cata1 like N'%"></c:set>
</c:forEach>
<c:set var="t2_end" value=" group by E_name) C2"></c:set>
<c:set var="sqlString" value="${sqlString}${t2_end}"></c:set>

<c:set var="t1_end1" value=" where P.E_name=C1.E_name and C1.c='${t1_count}'"></c:set>
<c:set var="t2_end1" value=" and P.E_name=C2.E_name and C2.c='${t2_count}'"></c:set>
<c:set var="sqlString" value="${sqlString}${t1_end1}${t2_end1}"></c:set>

<c:forEach var="price_c" items='<%=request.getParameterValues("type2[]")%>'>
<c:set var="sqlString" value="${sqlString}${price_begin}${price_c}"></c:set>
<c:set var="price_begin" value=" )or (price "></c:set>
</c:forEach>
<c:set var="sqlString" value="${sqlString}${price_end}"></c:set>
	<!--${sqlString}-->
	
<sql:setDataSource var="ds" dataSource="jdbc/TableGameDB" />
<sql:query sql="${sqlString}" var="rs" dataSource="${ds}" />
