...
<c:set var="g_maker" value="${param.g_maker}"></c:set>
<c:set var="iss" value="${param.iss}"></c:set>
<c:set var="sql" value="select * from product where c_name like N'%${c_name}%'"></c:set>

<sql:setDataSource var="ds" dataSource="jdbc/TableGameDB" />

<c:if test="${param.e_name != ''}">
<c:set var="sql" value="${sql} and e_name like N'%${e_name}%'"></c:set>
</c:if>

<c:if test="${param.g_maker != ''}">
<c:set var="sql" value="${sql} and g_maker like N'%${g_maker}%'"></c:set>
</c:if>

<c:if test="${param.iss != ''}">
<c:set var="sql" value="${sql} and iss like N'%${iss}%'"></c:set>
</c:if>

<sql:query sql="${sql}" var="rs" dataSource="${ds}" />            
...
<head>
	...
	<ul class="standard_ul">
            ...
            <li>遊戲作者 <img src="image/箭頭.png" class="standard_imgst" onclick="Gmaker_test()">
            ...
            <li>插畫家 <img src="image/箭頭.png" class="standard_imgst" onclick="Iss_test()">
            ...
            <li>遊戲人數
                ...
                <form id="fid7" action=Catch_Standard_JSP_test6.jsp method="get" style="display:none;font-size:smaller";>
            ...
            <li>遊戲價格
                ...
                <form id="fid8" action=Catch_Standard_JSP_test7.jsp method="get" style="display:none;font-size:smaller";>
...
<h2 style="color:beige">搜尋結果 :  <c:out value="${rs.rowCount}"></c:out> 筆資料</h2>
  <table border="1" style="background-color:beige;font-size:x-large;text-align:center">
    ...
    <td style="width:50px"><button type="submit" value="${row.c_name}" onclick="warnning()" name="delete">刪除</td>
    </form>
    <form>
    <td style="width:50px"><input  type="button" onclick="javascript:location.href='http://10.31.25.26:8080/Project/Standard/editgame.jsp?e_name=${row.e_name}'" value="修改"></input></td>
    </tr>
    </form>
    </c:forEach>
